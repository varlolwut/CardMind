...
The complete controls reference is available on-device under **Fn+4 → Help**. The carousel contains Chats, AI, Voice, Network, Files, Device, and Help cards. Browse it with the printed Left/Right arrow keys (plain `,` and `/`), press Enter to open a card, and press plain `` ` `` (the Esc-marked key) to go back. Inside lists, the plain arrow-marked `;` and `.` keys move the selection and Enter confirms it. In the chat manager, Fn+Delete opens a confirmation screen before deleting the selected chat. Wi-Fi, microSD, and battery status remain visible in the carousel header; battery level is also shown in the chat header. **Device → Web console** starts a protected console on the current Wi-Fi network. Open the local address shown on the Cardputer and sign in with the installation password displayed only on its screen. The console mirrors chat history, streams prompts, creates, switches, renames, or deletes chats, and edits per-chat instructions. It can also change the non-secret OpenAI-compatible base URL and model, browse large microSD workspace files in 12,288-byte chunks, edit a chunk atomically, download, rename, or delete a file. API keys remain write-only and are never returned to the browser. Press Esc on the Cardputer to close the console without restarting. Sessions expire after 15 minutes of inactivity and mutating requests require a session-specific CSRF token.
